...
export default function SalesPage() {
    ...
    const handlePreOrder = async () => {
        if (!customer) {
            alert(t("select_customer"));
            return;
        }
        if (cart.length === 0) {
            alert(t("empty_cart_error"));
            return;
        }
    ...
    return (
        <div className="flex flex-col md:flex-row h-[100dvh] md:h-[calc(100vh-2rem)] gap-0 md:gap-6 overflow-hidden bg-header dark:bg-[#0B0D0F] md:bg-transparent md:dark:bg-transparent pb-20 md:pb-0">
            ...
            <div className={cn(
                ...
                <div className="bg-card p-4 md:rounded-2xl shadow-sm border-b md:border border-border flex gap-4 items-center flex-none sticky top-0 z-40">
                    <div className="relative flex-1">
                        ...
                        <input
                            ...
                            autoFocus
                        />
                        {scanError && <div className="absolute top-full left-0 mt-1 text-xs font-bold text-debt animate-pulse bg-red-50 px-2 py-1 rounded">{scanError}</div>}
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                    <div className="mb-4 flex items-center gap-2 text-muted">
                        ...
                        <span className="text-xs font-bold uppercase tracking-widest">{t("product_catalog")}</span>
                    </div>
            ...
            <div className={cn(
                ...
                {/* Customer Section */}
                <div className="bg-card dark:bg-[#131517] p-5 rounded-2xl shadow-sm border border-border flex-none z-20 relative">
                    <h2 className="text-xs font-bold text-muted uppercase tracking-widest mb-3 flex items-center gap-2">
                        <Users className="w-4 h-4" /> {t("assigned_customer")}
                    </h2>
                    ...
                    {customer ? (
                        <div className="flex justify-between items-center bg-blue-50 dark:bg-blue-500/5 p-3 rounded-xl border border-blue-100 dark:border-blue-500/20">
                            <div className="overflow-hidden">
                                <p className="font-black text-primary dark:text-blue-300 uppercase truncate text-sm leading-tight">{customer.name}</p>
                                {customer.companyName && (
                                    <p className="text-xs font-bold text-transfer uppercase truncate mb-1 bg-blue-50 dark:bg-blue-900/20 w-fit px-1.5 py-0.5 rounded-sm">{customer.companyName}</p>
                                )}
                                <p className="text-[10px] font-mono text-muted">{customer.taxId}</p>
                                {customer.address && (
                                    <div className="flex items-center gap-1 text-[10px] text-secondary mt-1 truncate">
                            ...
                            <div className="flex gap-1 ml-2">
                                <button
                                    ...
                                    className="p-1.5 bg-card text-blue-600 rounded-lg hover:bg-blue-100 transition-colors shadow-sm border border-blue-100"
                                ...
                                <div className="relative flex-1">
                                    ...
                                    {foundCustomers.length > 0 ? (
                                            ...
                                            {foundCustomers.map(c => (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center justify-between mt-1">
                                                        ...
                                                        <p className="text-[10px] font-mono text-secondary">{c.taxId}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                        ...
                                        <div
                                            ...
                                            className="absolute top-full left-0 right-0 mt-2 bg-card rounded-xl shadow-xl border border-blue-100 dark:border-blue-500/20 z-50 p-3 cursor-pointer hover:bg-blue-50 dark:hover:bg-blue-500/10 group transition-all"
                                        >
                                            <p className="text-xs text-muted font-medium text-center group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                                                {t("create_customer").split('?')[0]}? <br />
                                                <span className="font-bold underline">{t("create_customer").split('?')[1]?.trim()}</span>
                                            </p>
                                        </div>
                                    )}
                                </div>
                ...
                <div className="flex-1 bg-card dark:bg-[#131517] text-primary rounded-2xl flex flex-col shadow-2xl overflow-hidden border border-border">
                    {/* Header */}
                    <div className="p-5 bg-slate-50/50 dark:bg-black/20 backdrop-blur-xl border-b border-border flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="font-black uppercase tracking-tight text-primary ">{t("current_cart")}</span>
                        </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                        {cart.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-muted opacity-50 dark:opacity-40 space-y-2">
                                ...
                                <p className="text-sm">{t("empty_cart")}</p>
                            </div>
                        ) : (
                                ...
                                <div key={`${item.product.id}-${idx}`} className="bg-header backdrop-blur-md rounded-2xl p-6 border border-border flex gap-6 group relative hover:bg-card /10 transition-all items-center shadow-sm dark:shadow-none">
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col gap-2">
                                        ...
                                        {item.serials && item.serials.length > 0 ? (
                                            <div className="flex flex-col gap-2 mt-1">
                                                <div className="flex flex-wrap gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        className="text-xs bg-blue-50 dark:bg-blue-500/10 text-transfer px-3 py-1 rounded-md hover:bg-blue-100 dark:hover:bg-blue-500/20 transition-colors font-bold border border-blue-100 dark:border-blue-500/20"
                                                    >
                                                        + SER
                                                    </button>
                                                </div>
                                                <button
                                                    ...
                                                    className="flex items-center gap-1 text-[10px] font-bold text-secondary hover:text-blue-600 w-fit transition-colors"
                                                >
                                            ...
                                            <button
                                                onClick={() => openSerialModal(item.product, idx)}
                                                className="self-start text-[10px] font-bold bg-blue-50 dark:bg-blue-500/10 text-transfer border border-blue-200 dark:border-blue-500/20 px-3 py-1 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-500/20 transition-colors uppercase mt-1"
                                            >
                                                Asignar Serial
                                            </button>
                                        )}
                                    ...
                                    <div className="flex flex-col items-end gap-3">
                                        <div className="relative group/price">
                                            ...
                                            <input
                                                ...
                                                className="w-48 pl-8 pr-4 py-3 bg-card dark:bg-black/40 border-2 border-border rounded-xl text-right font-black text-xl text-primary focus:border-blue-500 focus:outline-none focus:ring-4 focus:ring-blue-500/10 dark:focus:ring-blue-500/20 transition-all shadow-sm dark:shadow-none group-hover/price:border-blue-200 dark:group-hover/price:border-blue-500/50"
                                            />
                                        </div>
                                    ...
                                    <button
                                        ...
                                        className="absolute -top-3 -right-3 bg-card text-secondary hover:text-red-600 hover:bg-red-50 border border-border transition-all p-2 rounded-full shadow-lg opacity-0 group-hover:opacity-100 hover:scale-110 z-10"
                                    >
                                        <Trash2 className="w-4 h-4" />
                                    </button>
                                </div>
            ...
            <div className="md:hidden flex-none bg-card border-t border-border p-4 sticky bottom-0 z-50 pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] dark:shadow-none">
                <div className="flex justify-between items-end mb-3">
                    <div>
                        <p className="text-xs font-bold text-muted uppercase">Total a Pagar</p>
                        <p className="text-heading text-primary">${finalTotal.toLocaleString()}</p>
                    </div>
                ...
                {viewMode === 'CART' && (
                    <button onClick={() => setViewMode('CATALOG')} className="mt-3 w-full py-2 text-secondary font-bold text-xs uppercase hover:text-secondary">
                        Seguir Comprando
                    </button>
                )}
            </div>
