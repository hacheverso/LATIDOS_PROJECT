            ...
            if (item.serials && item.serials.length > 0) {
                        ...
                        if (warrantyAction) {
                            ...
                            await tx.instance.update({
                                ...
                                data: {
                                    ...
                                    warrantyNotes: warrantyAction.note ? `[${new Date().toLocaleDateString("en-US")} - ${userName}]: ${warrantyAction.note}` : undefined,
                                    returnDate: new Date(),
                                    returnBy: userName
                                } as any
                            });
                ...
                for (const serial of serialsToAdd) {
                    ...
                    await tx.instance.update({
                        ...
                        data: {
                            ...
                            warrantyNotes: (item as any).warrantyNote ? `[${new Date().toLocaleDateString("en-US")} - ${userName}]: ${(item as any).warrantyNote}` : undefined
                        } as any
                    });
                }
                ...
                if (serialsToKeep.length > 0) {
                    ...
                    if ((item as any).warrantyNote) {
                        ...
                        await tx.instance.updateMany({
                            where: { saleId: currentSale.id, productId: item.productId, serialNumber: { in: serialsToKeep } },
                            data: { warrantyNotes: `[${new Date().toLocaleDateString("en-US")} - ${userName}]: ${(item as any).warrantyNote}` } as any
                        });
                    }
                }
