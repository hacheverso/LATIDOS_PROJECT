    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 animate-in fade-in duration-200">
            <div className="bg-card w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200 ring-1 ring-white/20">
                {/* Header */}
                <div className="px-8 py-6 border-b border-border flex items-center justify-between bg-slate-50 dark:bg-white/5/80 backdrop-blur-xl">
                    <div>
                        <h2 className="text-subheading text-primary uppercase tracking-tight flex items-center gap-2">
                            <Sparkles className="w-5 h-5 text-blue-600" />
                            Crear Nuevo Producto
                        </h2>
                        <p className="text-xs text-secondary font-bold uppercase mt-1">
                            Vinculación Rápida
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                        className="p-3 -mr-2 rounded-full bg-header text-secondary hover:bg-red-100 hover:text-red-600 hover:scale-110 transition-all border border-border hover:border-red-200 hover:shadow-lg hover:shadow-red-500/20"
                        title="Cerrar (ESC)"
                    >
