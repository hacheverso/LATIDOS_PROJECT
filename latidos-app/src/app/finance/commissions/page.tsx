...
export default async function CommissionsPage() {
        ...
        <div className="w-full space-y-8 pb-20">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-heading text-primary tracking-tighter uppercase transition-colors">
                            Comisiones
                        </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-background p-6 rounded-2xl border border-border shadow-sm transition-colors">
                    ...
                    <p className="text-heading text-primary transition-colors">{formatMoney(globalSales)}</p>
                </div>
                <div className="bg-background p-6 rounded-2xl border border-border shadow-sm transition-colors">
                    <div className="flex items-center gap-3 mb-2">
  <div className="p-2 bg-green-50 dark:bg-brand  text-green-600 dark:text-black rounded-lg transition-colors">
                    ...
                    <p className="text-heading text-green-600 dark:text-emerald-400 transition-colors">{formatMoney(globalProfit)}</p>
                </div>
                <div className="bg-gradient-to-br from-purple-600 dark:from-purple-900/40 to-purple-800 dark:to-purple-900/20 text-white p-6 rounded-2xl shadow-lg border border-purple-500/50 dark:border-purple-500/20 transition-colors">
                    ...
                    <p className="text-heading text-white dark:text-purple-100 transition-colors">{formatMoney(globalCommissions)}</p>
                </div>
            </div>
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-header transition-colors">
                            <tr>
                                ...
                                <th className="px-6 py-4 text-right font-bold text-muted uppercase text-[10px] tracking-wider transition-colors">Total Vendido</th>
                            ...
                            {report.map(row => (
                                <tr key={row.id} className="hover:bg-hover/50 /5 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-center">
                                        <span className="px-2 py-1 bg-card-hover rounded text-xs font-bold text-secondary transition-colors">{row.saleCount}</span>
                                    </td>
                                    <td className="px-6 py-4 text-right font-mono font-medium text-secondary transition-colors">
                                        {formatMoney(row.totalSales)}
                                    </td>
                                    <td className="px-6 py-4 text-right font-mono text-muted transition-colors">
