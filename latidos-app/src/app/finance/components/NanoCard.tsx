...
const TYPE_BOX_COLORS: Record<string, string> = {
    CASH: "bg-emerald-50 dark:bg-emerald-500/10 text-success ring-emerald-100 dark:ring-emerald-500/20",
...
export function NanoCard({ account, onArchive, onEdit, onDelete, onRestore, isHighestLiquidity }: NanoCardProps) {
    ...
    return (
        <div className={cn(
            ...
            "bg-card/80 dark:bg-background border-border backdrop-blur-xl shadow-[0_10px_15px_-3px_rgba(0,0,0,0.05),0_4px_6px_-2px_rgba(0,0,0,0.02)] hover:border-slate-300 dark:hover:border-white/20",
                ...
                <div className="flex items-start justify-between">
                    ...
                    <Popover open={isOpen} onOpenChange={setIsOpen}>
                        <PopoverTrigger asChild>
                            <button className="text-muted hover:text-slate-700 dark:hover:text-slate-300 opacity-0 group-hover:opacity-100 transition-all p-1 rounded-md hover:bg-slate-100 dark:hover:bg-card/5 ml-auto">
                                <MoreVertical className="w-5 h-5" />
                            </button>
                        </PopoverTrigger>
                        <PopoverContent className="w-40 p-1 bg-background border-border shadow-xl" align="end">
                            <div className="flex flex-col space-y-0.5">
                                <Button variant="ghost" size="sm" className="justify-start h-8 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-card/5 hover:text-slate-900 dark:hover:text-white" onClick={() => { onEdit(account); setIsOpen(false); }}>
                                    <Pencil className="w-3 h-3 mr-2" /> Editar
                                </Button>

                                {isArchived ? (
                                    <Button variant="ghost" size="sm" className="justify-start h-8 text-xs font-semibold text-emerald-600 dark:text-success hover:bg-emerald-50 dark:hover:bg-emerald-500/10 hover:text-emerald-700 dark:hover:text-emerald-400" onClick={() => { onRestore?.(account.id); setIsOpen(false); }}>
                                        <ArrowUpRight className="w-3 h-3 mr-2" /> Restaurar
                                    </Button>
                                ) : (
                                    <Button variant="ghost" size="sm" className="justify-start h-8 text-xs font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-card/5 hover:text-slate-900 dark:hover:text-white" onClick={() => { onArchive(account.id); setIsOpen(false); }}>
            ...
            <div className="pt-3 mt-4 border-t border-slate-100/60 dark:border-white/5 bg-slate-50/50 dark:bg-card/[0.02] flex flex-wrap items-center gap-2 justify-between px-5 pb-3 mt-auto transition-colors">
