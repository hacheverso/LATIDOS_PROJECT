...
export default function CreditHistoryTable({ transactions }: CreditHistoryTableProps) {
    return (
        <div className="bg-white dark:bg-card rounded-2xl shadow-sm border border-slate-100 dark:border-white/10 flex flex-col h-full overflow-hidden mt-8">
            <div className="p-6 md:p-8 border-b border-slate-50 dark:border-white/5">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm font-medium text-slate-400 dark:text-slate-500">Movimientos de la billetera del cliente</p>
                    </div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-auto">
                {transactions.length === 0 ? (
                    <div className="flex flex-col items-center justify-center p-8 text-center">
                        ...
                        <p className="text-slate-500 dark:text-slate-400 font-medium">No hay movimientos de saldo registrados.</p>
                    </div>
                ) : (
                    <table className="w-full text-left text-sm">
                        <thead className="bg-slate-50/80 dark:bg-[#1A1C1E]/80 sticky top-0 z-10 backdrop-blur">
                            <tr>
                                <th className="px-6 py-4 font-black text-slate-400 dark:text-slate-500 uppercase text-xs tracking-wider">Fecha</th>
                                <th className="px-6 py-4 font-black text-slate-400 dark:text-slate-500 uppercase text-xs tracking-wider">Concepto</th>
                                <th className="px-6 py-4 font-black text-slate-400 dark:text-slate-500 uppercase text-xs tracking-wider text-right">Monto</th>
                            </tr>
                        </thead>
                            ...
                            {transactions.map((tx) => (
                                <tr key={tx.id} className="hover:bg-slate-50/50 dark:hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="px-6 py-4">
                                        ...
                                        {tx.referenceId && <div className="text-xs text-slate-400 dark:text-slate-500 font-mono">Ref: {tx.referenceId.slice(0, 8)}</div>}
                                    </td>
