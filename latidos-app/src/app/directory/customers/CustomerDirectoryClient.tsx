    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500 min-h-screen bg-slate-50/50 dark:bg-transparent p-4 md:p-8 transition-colors">
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-primary tracking-tight">Directorio de Clientes</h1>
                    <p className="text-muted text-sm">Gestiona tu base de datos de clientes y su estado financiero.</p>
                </div>
                <div className="flex items-center gap-3">
                    <button
                        onClick={() => setIsImportModalOpen(true)}
                        className="bg-card/5 hover:bg-hover /10 text-primary  border border-border text-sm font-medium py-2.5 px-5 rounded-xl transition-colors flex items-center gap-2 shadow-sm"
                    >
                    ...
                    <button
                        ...
                        className="bg-card dark:bg-card hover:bg-slate-800 dark:hover:bg-slate-200 dark:bg-card/10 text-white dark:text-primary text-sm font-medium py-2.5 px-5 rounded-xl transition-colors flex items-center gap-2 shadow-lg shadow-slate-900/10 dark:shadow-none"
                    >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="bg-card p-4 rounded-xl border border-border shadow-sm transition-colors">
                    <div className="flex items-center gap-3 mb-2">
                        <div className="p-2 bg-blue-50 dark:bg-blue-500/10 text-transfer rounded-lg">
                            <Users className="w-5 h-5" />
                        </div>
                        <span className="text-xs font-semibold text-muted uppercase tracking-wider">Total Clientes</span>
                    </div>
                    <div className="text-2xl font-bold text-primary">{metrics.totalRegistered}</div>
                </div>
                ...
                <div className="bg-card p-4 rounded-xl border border-border shadow-sm transition-colors">
                    ...
                    <div className="text-sm font-bold text-primary truncate" title={metrics.topClientName}>{metrics.topClientName || 'N/A'}</div>
                ...
                <div className="bg-card p-4 rounded-xl border border-border shadow-sm transition-colors">
                    ...
                    <div className="text-2xl font-bold text-primary">{formatMoney(metrics.averageTicket)}</div>
                </div>
                ...
                <div className="bg-card p-4 rounded-xl border border-border shadow-sm transition-colors">
                    ...
                    <div className="text-2xl font-bold text-primary">{metrics.coverageValue || "100%"}</div>
