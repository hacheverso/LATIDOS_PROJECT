...
export default function SalesPage() {
    ...
    const handleScan = async (e: React.KeyboardEvent<HTMLInputElement>) => {
        if (e.key === 'Enter' && scanInput) {
            ...
            if (cart.some(item => item.serialNumber === scanInput)) {
                setScanError("Este producto ya está en el carrito.");
                setScanInput("");
                return;
            }
    ...
    // --- Cart Logic ---
    const addToCart = (instance: any) => {
        // Double check if already in cart (for Catalog clicks)
        if (cart.some(item => item.serialNumber === instance.serialNumber)) {
            alert("Este serial ya está en el carrito"); // Fallback for manual selection
            return;
        }
        setCart(prev => [...prev, instance]);
    };
    ...
    // --- Cart Totals ---
    const total = cart.reduce((acc, item) => acc + Number(item.product.basePrice), 0);
    ...
    const handleCheckout = async () => {
        ...
        try {
            await processSale({
                ...
                items: cart.map(i => i.serialNumber),
                total: total,
                paymentMethod
            });
            setSaleSuccess(true);
            ...
            <div className="flex-1 flex flex-col gap-4 h-full min-w-[380px]">
                ...
                <div className="flex-1 bg-slate-900 text-white rounded-2xl flex flex-col shadow-2xl overflow-hidden">
                    ...
                    <div className="p-5 bg-slate-950/50 border-b border-white/5 flex items-center justify-between">
                        ...
                        <Badge className="bg-blue-600 text-white border-0">{cart.length}</Badge>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                        {cart.length === 0 ? (
                            ...
                            cart.map((item, idx) => (
                                <div key={`${item.serialNumber}-${idx}`} className="bg-white/5 rounded-xl p-3 border border-white/5 flex gap-3 group relative hover:bg-white/10 transition-colors">
                                    <div className="flex-1 min-w-0">
                                        <p className="font-bold text-sm text-slate-200 truncate">{item.product.name}</p>
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="text-[10px] font-mono bg-slate-800 text-slate-400 px-1.5 py-0.5 rounded">{item.serialNumber}</span>
                                            <span className="text-xs text-slate-300 font-bold ml-auto">${Number(item.product.basePrice).toLocaleString()}</span>
                                        </div>
                                    </div>
                                    <button
                                        onClick={() => setCart(cart.filter((_, i) => i !== idx))}
                                        className="text-slate-500 hover:text-red-400 transition-colors self-center p-1"
                                    >
